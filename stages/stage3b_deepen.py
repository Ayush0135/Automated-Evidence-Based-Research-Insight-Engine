...
def stage3b_deepen_research(analyzed_docs, topic):
    ...
    new_queries_list = extract_json(response)
    if not isinstance(new_queries_list, list):
        new_queries_list = []
    ...
    # Extract existing urls and titles to avoid duplicate discovery and downloads
    existing_urls = {d['url'] for d in analyzed_docs if 'url' in d}
    existing_titles = {d['title'].lower().strip() for d in analyzed_docs if 'title' in d}
