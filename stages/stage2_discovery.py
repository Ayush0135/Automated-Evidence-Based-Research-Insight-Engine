...
def stage2_document_discovery(decomposition_data):
    ...
    Executes search queries in parallel and downloads content.
    Maintains document relevance order by preserving input sequence during result gathering.
    """
    print("\n--- STAGE 2: DOCUMENT DISCOVERY (Order-Preserving) ---")
    ...
    with ThreadPoolExecutor(max_workers=5) as search_executor:
        # Maintain order by iterating over submitted futures
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        # Preserve search ranking by iterating over submitted futures
