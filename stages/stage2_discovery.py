...
def stage2_document_discovery(decomposition_data):
    ...
    with ThreadPoolExecutor(max_workers=5) as search_executor:
        # Submit all search queries and keep track of futures in order
        futures = [search_executor.submit(execute_search_query, s, q) for s, q in all_queries]
        
        # Gather results by iterating over futures list to preserve order (most relevant first)
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        # Submit all download tasks and keep track of futures in order
        futures = [executor.submit(process_search_item, item) for item in search_candidates]
        
        # Gather results by iterating over futures list to preserve search ranking order
        for future in futures:
