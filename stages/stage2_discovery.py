...
def stage2_document_discovery(decomposition_data):
    ...
    with ThreadPoolExecutor(max_workers=5) as search_executor:
        # Submit all queries and keep track of futures in order
        futures = [search_executor.submit(execute_search_query, s, q) for s, q in all_queries]
        
        # Iterate over futures in submission order to preserve relevance of subtopics/queries
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        # Submit all items and keep track of futures in order
        futures = [executor.submit(process_search_item, item) for item in search_candidates]
        
        # Iterate over futures in submission order to preserve the original relevance ranking
        for future in futures:
