    ...
    try:
        ...
        if len(full_text) > 12000:
            ...
            with ThreadPoolExecutor(max_workers=3) as chunk_executor:
                ...
                # Iterating over futures in submission order to preserve the sequence of chunks
                for f in futures:
                    res = f.result()
...
def stage3_document_analysis(documents):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        # Submit all documents and keep track of futures in order
        futures = [executor.submit(analyze_single_document, doc) for doc in documents]
        
        # Iterating over futures in submission order to preserve document order
