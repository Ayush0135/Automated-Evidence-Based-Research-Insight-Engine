...
def analyze_single_document(doc):
    try:
        ...
        if len(full_text) > 12000:
            ...
            # Using 3 workers for chunk analysis. Parallelization with ordered gathering ensures
            # the final document summary preserves the logical chronology of the text.
            with ThreadPoolExecutor(max_workers=3) as chunk_executor:
                ...
                # Gather results by iterating over futures in order (avoids scrambled chronology)
                for f in futures:
                    res = f.result()
...
def stage3_document_analysis(documents):
    ...
    # Process documents in parallel while preserving input sequence
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        # Gather results in the same order as submitted to maintain relevance ranking
