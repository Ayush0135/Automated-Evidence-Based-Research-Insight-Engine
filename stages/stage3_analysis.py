    ...
    try:
        ...
        if len(full_text) > 12000:
            ...
            with ThreadPoolExecutor(max_workers=3) as chunk_executor:
                # Store futures in list to preserve chronological chunk order
                futures = [chunk_executor.submit(analyze_chunk, i, c) for i, c in enumerate(selected_chunks)]
                # Iterate in order to maintain context sequence
                for f in futures:
                    res = f.result()
...
def stage3_document_analysis(documents):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        # Preserving relevance order by iterating over futures in submission sequence
