...
def score_single_document(doc, topic):
    ...
    try:
        ...
        print(f"  Error scoring document {doc.get('title', '')[:20]}: {e}")
        return None
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # max_workers=3 is a balanced choice for speed vs rate limits
    ...
    # Sort by score descending (optional, but good for consistent processing later)
    scored_documents.sort(key=lambda x: x['scoring'].get('score', 0), reverse=True)
