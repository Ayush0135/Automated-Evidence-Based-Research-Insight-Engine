...
def score_single_document(doc, topic):
    """
    Worker function to score a single document.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelizes document scoring using ThreadPoolExecutor.
    Preserves document order by gathering results in submission order.
    Expected Speedup: ~3x with 3 workers for I/O bound LLM calls.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallel) ---")
    scored_documents = []
    
    # Parallelizing scoring with 3 workers to balance throughput and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        # Gather results in original order to preserve relevance-based ranking
