...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Stage 4 using ThreadPoolExecutor to speed up scoring.
    Preserves document order for consistency in downstream filtering.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallel) ---")
    scored_documents = []
    
    # Use 3 workers to balance throughput and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit tasks and maintain their order
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gather results in the order they were submitted
