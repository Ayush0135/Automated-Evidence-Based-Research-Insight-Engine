...
def score_single_document(doc, topic):
    """
    Helper function to score a single document in parallel.
    """
    analysis = doc.get('analysis', {})
    if not analysis:
        return None

    # No print here to avoid messy logs during parallel execution
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Use ThreadPoolExecutor for parallel scoring (I/O bound API calls)
    # max_workers=3 balance speed and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit tasks
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Iterate over futures in submission order to preserve document relevance
        for future in futures:
            result = future.result()
            if result:
                scored_documents.append(result)
                print(f"  Score: {result['scoring'].get('score')} for {result['title'][:30]}...")
