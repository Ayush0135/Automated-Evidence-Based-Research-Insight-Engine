...
def score_single_document(doc, topic):
    ...
    Extracted for parallelization in Stage 4.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Stage 4: Academic Scoring.
    Uses ThreadPoolExecutor to score multiple documents concurrently.
    Preserves document order to maintain relevance ranking.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Groq) ---")
    scored_documents = []
    
    # BOLT OPTIMIZATION: Use 3 workers for ~3x throughput improvement while managing rate limits.
    # We iterate over futures in submission order to preserve the original document ranking.
