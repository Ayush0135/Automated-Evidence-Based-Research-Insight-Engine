...
def score_single_document(doc, topic):
    """
    Helper function to score a single document using LLM.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Use ThreadPoolExecutor to score documents in parallel
    # 3 workers is a safe balance for Groq rate limits
    scored_documents = []
    
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks and keep track of them in order
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gathering results in the original order
