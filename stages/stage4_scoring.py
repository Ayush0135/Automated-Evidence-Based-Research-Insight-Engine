...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Using 3 workers as per performance guidelines to balance speed and rate limits
    # Maintaining order by iterating over submitted futures
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gathering results in submission order to preserve document ranking
