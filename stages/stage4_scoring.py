...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized academic scoring of documents.
    Preserves the original document order (based on search relevance).
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallelized) ---")
    scored_documents = []
    
    # Stage 4 is parallelized using ThreadPoolExecutor with 3 workers.
    # Results are gathered by iterating over futures in submission order to
    # preserve the relevance-based ranking established in Stage 2.
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks and keep them in a list to preserve order
