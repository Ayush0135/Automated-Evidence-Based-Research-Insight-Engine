...
def score_single_document(doc, topic):
    """
    Helper to score a single document using Groq.
    """
    ...
    try:
        ...
        print(f"  Error scoring {doc['title'][:20]}: {e}")
        return None
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # max_workers=3 as per performance memory to balance speed and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Iterate over futures in ORDER of submission to maintain relevance-based ranking
