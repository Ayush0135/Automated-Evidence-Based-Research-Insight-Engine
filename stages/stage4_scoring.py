...
def score_single_doc(doc, topic):
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Optimization: Use ThreadPoolExecutor to score documents in parallel.
    # We use 3 workers to balance speed and rate limits.
    # We iterate over futures in submission order to preserve document ranking.
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = [executor.submit(score_single_doc, doc, topic) for doc in analyzed_documents]
