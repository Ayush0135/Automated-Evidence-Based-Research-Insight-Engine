...
def score_single_document(doc, topic):
    ...
    try:
        ...
        print(f"  + Scored: {doc['title'][:40]}... (Score: {score_data.get('score')})")
        return doc
...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Stage 4: Scores documents using ThreadPoolExecutor.
    Uses 3 workers to speed up scoring while maintaining order.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallel) ---")
    scored_documents = []
    
    # max_workers=3 for balanced throughput and API limits
