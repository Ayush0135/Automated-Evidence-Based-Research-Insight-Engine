...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Academic Scoring using ThreadPoolExecutor.
    Preserves input order by gathering futures in submission sequence.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallel Groq) ---")
    scored_documents = []
    
    # Use ThreadPoolExecutor for parallel scoring
    # 3 workers is a safe default to avoid aggressive rate limiting while providing speedup
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all scoring tasks
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gather results in order of submission to preserve relevance ranking
