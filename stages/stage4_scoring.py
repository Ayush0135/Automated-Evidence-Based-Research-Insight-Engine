...
def score_single_document(doc, topic):
    """
    Helper to score a single document using an LLM.
    """
    ...
    try:
        ...
        print(f"  x Error scoring {doc['title'][:30]}: {e}")
        return None
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Use ThreadPoolExecutor for parallel scoring to reduce total latency.
    # max_workers=3 is chosen as a safe default for most API rate limits.
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks and keep track of futures to maintain order
