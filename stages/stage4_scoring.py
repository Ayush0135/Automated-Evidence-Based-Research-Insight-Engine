...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Stage 4 using ThreadPoolExecutor to handle I/O bound LLM calls.
    Preserves document order to maintain relevance ranking.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Groq) ---")
    scored_documents = []
    
    # 3 workers is a safe default to avoid aggressive rate limiting while gaining speed
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit tasks and keep track of futures to preserve order
