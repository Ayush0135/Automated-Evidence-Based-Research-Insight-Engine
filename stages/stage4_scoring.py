...
def score_single_document(doc, topic):
    ...
    try:
        ...
        # print(f"  Score for {doc['title'][:20]}: {score_data.get('score')}")
        return doc
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Using ThreadPoolExecutor for parallel scoring.
    # max_workers=3 is a balance between speed and rate limit friendliness.
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks and keep track of futures
        # We store them in a list to maintain submission order if needed,
        # but here we'll just collect them as they complete or in order.
        # To strictly maintain order of analyzed_documents in scored_documents:
        ...
        for future in future_to_doc:
            ...
            if result:
                ...
                print(f"  + Scored: {result['title'][:50]} (Score: {result['scoring'].get('score')})")
