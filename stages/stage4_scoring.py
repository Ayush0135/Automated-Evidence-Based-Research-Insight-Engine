...
def score_single_document(doc, topic):
    """
    Helper function to score a single document.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Bolt: Parallelize scoring to speed up the stage
    # max_workers=3 is a balanced number for rate limits and throughput
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Maintain order by storing futures in a list
        future_to_doc = {executor.submit(score_single_document, doc, topic): doc for doc in analyzed_documents}
        
        # We want to preserve the original order if possible, but as_completed is faster for finishing.
        # Actually, for scoring, order doesn't strictly matter as they will be filtered/sorted later,
        # but let's use submission order if we want to be safe.
        # However, as_completed is fine here as we collect them all.
