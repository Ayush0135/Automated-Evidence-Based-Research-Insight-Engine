...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Use 3 workers as recommended for balanced throughput and rate-limit safety
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gather results in original order to preserve relevance ranking
        results = [f.result() for f in futures]
        
        # Filter out failed scorings
        scored_documents = [r for r in results if r is not None]
