...
def score_single_document(doc, topic):
    ...
    try:
        ...
        print(f"  Error scoring {doc.get('title', 'Unknown')[:30]}: {e}")
        return None
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Use 3 workers for a balance of speed and rate-limit safety
    # Iterating over futures in submission order ensures we maintain original ranking/order
