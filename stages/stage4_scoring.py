...
def score_single_document(doc, topic):
    """
    Worker function to score a single document using LLM.
    """
    ...
    try:
        ...
        print(f"  + Scored ({score_data.get('score')}): {doc['title'][:40]}...")
        return doc
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Parallelizing scoring calls (I/O bound) to Groq.
    # Uses 3 workers as per performance guidelines to balance speed and rate limits.
