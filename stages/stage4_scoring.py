...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # max_workers=3 balances speed and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all scoring tasks
        future_to_doc = {executor.submit(score_single_document, doc, topic): doc for doc in analyzed_documents}
        
        # Collect results as they complete
        # We iterate over the original list to preserve order if possible,
        # but as_completed is faster for finishing the stage.
        # Given this is a research pipeline, preserving order might be nice for consistency.
        # However, Stage 5 filters based on score anyway.
