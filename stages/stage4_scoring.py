...
def score_single_document(doc, topic):
    ...
    Ensures thread safety and cleaner logic for parallelization.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Academic Scoring of analyzed documents.
    Preserves document order to maintain relevance-based ranking.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Groq - Parallel) ---")

    # max_workers=3 for balanced throughput and rate limiting
    # Parallelizing this stage can reduce overall execution time significantly (up to 3x)
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        # Iterate over futures in the order they were submitted to maintain original list sequence
        # This ensures downstream stages (like filtering) receive documents in a predictable order
        scored_documents = []
