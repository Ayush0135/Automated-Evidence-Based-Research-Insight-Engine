...
def score_single_document(doc, topic):
    """
    Helper to score a single document via LLM.
    """
    ...
    try:
        ...
        print(f"  Score: {score_data.get('score')} for {doc['title'][:20]}...")
        return doc
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # 3 workers to balance throughput and API rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gather results in SUBMISSION ORDER to preserve original relevance-based ranking
