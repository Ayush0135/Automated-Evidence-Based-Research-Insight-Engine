...
def score_single_document(doc, topic):
    """
    Scores a single document using an LLM.
    """
    ...
    try:
        ...
        print(f"  Score: {score_data.get('score')} for {doc.get('title', 'Unknown')[:30]}")
        return doc
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # 3 workers provides a good balance between speed and rate-limit safety
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        # Iterate over futures in submission order to preserve document ranking/order
