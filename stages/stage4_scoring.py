...
def score_single_document(doc, topic):
    """
    Helper to score a single document using the LLM.
    Preserves document state by returning the modified document.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Stage 4. Uses ThreadPoolExecutor with 3 workers.
    Preserves original document order.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallel) ---")
    scored_documents = []
    
    # max_workers=3 to balance speed and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Map each doc to a scoring task
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Gather results in the same order as submitted to preserve ranking
