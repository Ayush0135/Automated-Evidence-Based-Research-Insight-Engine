...
def score_single_document(doc, topic):
    ...
    Ensures modularity for parallel execution.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    Performance Impact: ~3x speedup for typical batches of 10+ documents.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Groq - Parallel) ---")
    scored_documents = []
    
    # max_workers=3 is a balanced choice for Groq rate limits and throughput
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all scoring tasks
        futures = [executor.submit(score_single_document, doc, topic) for doc in analyzed_documents]
        
        # Iterate over futures in the order they were submitted to preserve relevance ranking
