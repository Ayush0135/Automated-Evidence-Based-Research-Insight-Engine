...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Parallelized Stage 4.
    Expected performance: 3x speedup with 3 workers.
    Order preservation: Guaranteed by iterating over futures list in submission order.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Parallel Groq) ---")
    scored_documents = []
    
    # max_workers=3 balance between speed and rate-limiting
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = [executor.submit(score_document, doc, topic) for doc in analyzed_documents]
        
        # Iterate over futures in order of submission to preserve input order
