...
def score_single_document(doc, topic):
    ...
    Encapsulates scoring logic and robust JSON parsing.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    """
    Stage 4: Evaluate documents based on academic criteria.
    Optimized: Parallelized using ThreadPoolExecutor to reduce execution time.
    """
    print("\n--- STAGE 4: ACADEMIC SCORING (Groq - Parallelized) ---")
    scored_documents = []
    
    # ⚡ BOLT OPTIMIZATION: Parallelize scoring to significantly reduce latency.
    # 3 workers selected to balance speed and provider rate limits.
