...
def score_single_document(doc, topic):
    ...
    try:
        ...
        print(f"  Error scoring document {doc.get('title', '')[:30]}: {e}")
        return None
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # Parallelize scoring with a limited number of workers to avoid rate limits
    # max_workers=3 is a balanced choice for Groq's free tier or typical API limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit tasks and collect results.
        # Using list comprehension with future.result() ensures we wait for all and preserve order.
