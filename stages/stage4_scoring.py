...
def score_single_document(doc, topic):
    """
    Scores a single document using the LLM.
    """
...
def stage4_academic_scoring(analyzed_documents, topic):
    ...
    # 3 workers is a reasonable default to balance speed and rate limits
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks and keep track of them
        # We use a list of futures to maintain order if we iterate over it
        # but here we use as_completed for slightly faster response handling
        # and then we'll re-sort if necessary, but typically Stage 5 handles rank.
        # Actually, let's keep the order of the input documents.
